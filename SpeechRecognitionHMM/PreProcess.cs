...
namespace SpeechRecognitionHMM
{
    ...
    public class PreProcess
    {
        ...
        /// <param name="samplePerFrame">
        ///     how many samples in one frame (660 << frameDuration = typically 30)</param>
        ...
        {
            var max = originalSignal[0];
            for (var i = 1; i < originalSignal.Length; i++)
                if (max < Math.Abs(originalSignal[i]))
                    max = Math.Abs(originalSignal[i]);

            // Console.Out.WriteLine("max PCM =  " + max);
            for (var i = 0; i < originalSignal.Length; i++) originalSignal[i] = originalSignal[i] / max;
        }
        ...
        private void DoFraming()
        {
            ...
            for (var i = 0; i < noOfFrames; i++) framedSignal[i] = new float[samplePerFrame];

            for (var i = 0; i < noOfFrames; i++)
            {
                var startIndex = i * samplePerFrame / 2;
                for (var j = 0; j < samplePerFrame; j++) framedSignal[i][j] = afterEndPtDetection[startIndex + j];
            }
        }
