namespace Wavelets
{
    ...
    {
        private readonly bool useOriginalTensorDecomposition = true;

        public StandardHaarWaveletDecomposition(bool useTensorDecomposition = true)
        {
            useOriginalTensorDecomposition = useTensorDecomposition;
        }
        ...
        /// </summary>
        /// <remarks>
        ///     Copied from the Soundfingerprinting project
        ...
        {
            var rows = matrix.Length; /*128*/
            var cols = matrix[0].Length; /*32*/
            ...
            for (var col = 0; col < cols /*32*/; col++)
            {
                ...
                for (var row = 0; row < rows; row++) column[row] = matrix[row][col]; /*Copying Column vector*/

                Decomposition(column); /*Decomposition of each row*/
                for (var row = 0; row < rows; row++) matrix[row][col] = column[row];
            }

            for (var row = 0; row < rows /*128*/; row++) Decomposition(matrix[row]); /*Decomposition of each row*/
        }
        ...
        private void DecomposeImage(double[][] matrix)
        {
            ...
            while (w > 1 || h > 1)
            {
                if (w > 1)
                    ...
                    for (i = 0; i < h; i++)
                    {
                        for (j = 0; j < cols; j++) temp_row[j] = matrix[i][j];

                        DecompositionStep(temp_row, w);

                        for (j = 0; j < cols; j++) matrix[i][j] = temp_row[j];
                    }
                ...
                if (h > 1)
                    ...
                    for (i = 0; i < w; i++)
                    {
                        for (j = 0; j < rows; j++) temp_col[j] = matrix[j][i];

                        DecompositionStep(temp_col, h);

                        for (j = 0; j < rows; j++) matrix[j][i] = temp_col[j];
                    }
