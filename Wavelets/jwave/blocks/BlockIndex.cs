...
namespace math.transform.jwave.blocks
{
    ...
    public class BlockIndex : Block
    {
        ...
        public BlockIndex(int offSetRow, int offSetCol, int noOfRows, int noOfCols) : base(offSetRow, offSetCol,
            noOfRows, noOfCols)
        {
        }
        ...
        public override double get(int i, int j)
        {
            ...
            {
                var exceptionMsg = blockException.getMessage();
            ...
            if (isOccupied)
            {
                var pos = getOccupiedInternalArrayIndices(i, j);
        ...
        public override double[][] get()
        {
            ...
            var matrix = MathUtils.CreateJaggedArray<double[][]>(_noOfRows, _noOfCols);

            var noOf = _arrVal.Count;

            for (var p = 0; p < noOf; p++)
