...
namespace Wavelets
{
    ...
    public class Dwt
    {
        private readonly double[] analysisHighPass = DivBySqrt2(new double[] { -1, 1 });
        ...
        private readonly double[] analysisLowPass = DivBySqrt2(new double[] { 1, 1 });
        private readonly int padding;
        private readonly int steps;
        private readonly double[] synthesisHighPass = DivBySqrt2(new double[] { 1, -1 });
        private readonly double[] synthesisLowPass = DivBySqrt2(new double[] { 1, 1 });
        ...
        private double[] TransformStep(double[] input, int signalSize)
        {
        ...
        private double[] TransformBackStep(double[] input)
        {
        ...
        private static void ExtendPeriodically(double[] input, int signalSize)
        {
            for (var i = signalSize; i < input.Length; i++)
                input[i] = input[i - signalSize];
        }

        private static double[] DivBySqrt2(IEnumerable<double> arr)
        {
            return arr.Select(x => x / Math.Sqrt(2)).ToArray();
        }

        private static double GetElementReversed(double[] arr, int index)
        {
