...
namespace Wavelets.Compress
{
    ...
    public static class HaarWaveletTransform
    {
        ...
        public static void HaarTransform1D(double[] data_input, int size)
        {
            ...
            for (i = 0; i < size; i++) data_input[i] = temp_data[i];
        ...
        public static void HaarTransform3D(double[][][] data_input, int length, int width, int height)
        {
            ...
            if (width > 1 && height > 1)
                ...
                for (j = 0; j < width; j++)
                {
                    for (k = 0; k < length; k++) temp_length[k] = data_input[k][j][i];

                    HaarTransform1D(temp_length, length);

                    for (k = 0; k < length; k++) data_input[k][j][i] = temp_length[k];
                }
            ...
            if (length > 1 && height > 1)
                ...
                for (j = 0; j < length; j++)
                {
                    for (k = 0; k < width; k++) temp_width[k] = data_input[j][k][i];

                    HaarTransform1D(temp_width, width);

                    for (k = 0; k < width; k++) data_input[j][k][i] = temp_width[k];
                }
            ...
            if (length > 1 && width > 1)
                ...
                for (j = 0; j < width; j++)
                {
                    for (k = 0; k < height; k++) temp_height[k] = data_input[i][j][k];

                    HaarTransform1D(temp_height, height);

                    for (k = 0; k < height; k++) data_input[i][j][k] = temp_height[k];
                }
