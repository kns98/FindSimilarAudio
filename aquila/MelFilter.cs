...
namespace Aquila
{
    ...
    public class MelFilter
    {
		 ...
		 */
        private readonly double sampFreq;
        ...
        public void CreateFilter(int filterNum, int melFilterWidth, int N)
        {
            ...
            var melMinFreq = filterNum * melFilterWidth / 2.0;
            var melCenterFreq = melMinFreq + melFilterWidth / 2.0;
            var melMaxFreq = melMinFreq + melFilterWidth;

            // converting them to linear
            var minFreq = MelToLinear(melMinFreq);
            var centerFreq = MelToLinear(melCenterFreq);
            var maxFreq = MelToLinear(melMaxFreq);
