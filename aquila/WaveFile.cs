...
namespace Aquila
{
    ...
    public class WaveFile
    {
		 ...
		 * Header structure.
		 */
        private readonly WaveHeader hdr = new WaveHeader();
        ...
        /**
         * Overlap between frames - fraction of frame length (0
         * < overlap
         * <
         * 1
         * )
        ...
        public void Load(string file)
        {
            ...
            var fs = new BinaryFile(filename);
            LoadHeader(ref fs);
            var data = new short[hdr.WaveSize / 2];
            LoadRawData(ref fs, data, hdr.WaveSize / 2);
            fs.Close();

            // initialize data channels (using right channel only in stereo mode)
            var channelSize = hdr.WaveSize / hdr.BytesPerSamp;
         ...
         */
        public uint GetSampleFrequency()
        {
            return hdr.SampFreq;
        }
        ...
        public void SaveFrames(string filename, int begin, int end)
        {
            ...
            var samples = GetSamplesPerFrame();

            // calculate the boundaries of a fragment of the source channel
            // which correspond to given frame numbers
            var startPos = (int)(begin * samples * (1 - overlap));
            var endPos = (int)((end + 1) * samples * (1 - overlap) + samples * overlap);
            if (endPos > LChTab.Count)
                endPos = LChTab.Count;

            // number of data bytes in the resulting wave file
            var waveSize = (uint)(endPos - startPos) * hdr.BytesPerSamp;

            // prepare a new header and write it to file stream
            var newHdr = new WaveHeader();
            //std.strncpy(newHdr.RIFF, hdr.RIFF, 4);
            newHdr.DataLength = (uint)(waveSize + newHdr.WaveHeaderSize);
            ...
            for (var i = 0; i < data.Length; i++) fs.Write(data[i]);
            fs.Close();
            data = null;
        }
        ...
        {
            var bytesPerFrame = (int)(hdr.BytesPerSec * frameLength / 1000.0);
         ...
         */
        private void LoadRawData(ref BinaryFile fs, short[] buffer, uint bufferLength)
        {
            for (var i = 0; i < bufferLength; i++) buffer[i] = fs.ReadInt16();
        }
        ...
        public void DivideFrames(List<short> source)
        {
            ...
            var samplesPerNonOverlap = (int)(samplesPerFrame * (1 - overlap));
            var framesCount = (int)(hdr.WaveSize / hdr.BytesPerSamp) / samplesPerNonOverlap;
            var power = (int)(Math.Log(samplesPerFrame) / Math.Log(2.0));
            zeroPaddedLength = 1 << (byte)(power + 1);
            ...
            for (int i = 0, size = source.Count; i < framesCount; ++i)
            {
                ...
                if (indexEnd < size) //frames.push_back(new Frame(source, indexBegin, indexEnd));
        ...
        {
            public ushort BitsPerSamp;
            public ushort BytesPerSamp;
            ...
            public uint SampFreq;
            public uint SubBlockLength;
