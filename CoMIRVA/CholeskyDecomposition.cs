...
namespace Comirva.Audio.Util.Maths
{
    ...
    public class CholeskyDecomposition
    {
        ...
        public CholeskyDecomposition(Matrix Arg)
        {
            ...
            for (var j = 0; j < n; j++)
            {
                ...
                for (var k = 0; k < j; k++)
                {
                    var Lrowk = L[k];
                    var s = 0.0;
                    for (var i = 0; i < k; i++) s += Lrowk[i] * Lrowj[i];
        ...
        {
            if (B.GetRowDimension() != n) throw new ArgumentException("Matrix row dimensions must agree.");
            if (!isspd) throw new Exception("Matrix is not symmetric positive definite.");
            ...
            for (var j = 0; j < nx; j++)
            {
                for (var i = 0; i < k; i++) X[k][j] -= X[i][j] * L[k][i];
                X[k][j] /= L[k][k];
            }
            ...
            for (var j = 0; j < nx; j++)
            {
                for (var i = k + 1; i < n; i++) X[k][j] -= X[i][j] * L[i][k];
                X[k][j] /= L[k][k];
            }
