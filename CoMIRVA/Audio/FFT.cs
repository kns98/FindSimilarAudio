...
namespace Comirva.Audio
{
    ...
    public class FFT
    {
        ...
        public FFT(int transformationType, int windowSize, double[] windowFunction)
        {
            ...
            if (windowFunction.Length != windowSize) throw new Exception("size of window function match window size");

            this.windowFunction = windowFunction;
            windowFunctionType = WND_USER_DEFINED;
        ...
        {
            var n = re.Length;
            var bits = (int)Math.Round(Math.Log(n) / Math.Log(2), 0);
        ...
        /// <param name="size">
        ///     the number of non zero values; if the array is larger than
        ///     this, it is zero-padded symmetrically at both ends
        /// </param>
        private void Hamming(int size)
        {
            var start = (windowFunction.Length - size) / 2;
            var stop = (windowFunction.Length + size) / 2;
            var scale = 1.0 / size / 0.54;
            var factor = PI2 / size;
        ...
        private void Gauss(int size)
        {
            ...
            var stop = (windowFunction.Length + size) / 2;
            var delta = 5.0 / size;
            var x = (1 - size) / 2.0 * delta;
            var c = -Math.PI * Math.Exp(1.0) / 10.0;
            double sum = 0;
        ...
        {
            windowFunctionSum = 0;
            for (var i = 0; i < windowFunction.Length; i++)
