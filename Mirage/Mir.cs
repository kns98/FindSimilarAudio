...
namespace Mirage
{
    public class Mir
    {
        ...
        public static string[] extensionsWithStar =
        {
            "*.wav", "*.ogg", "*.mp1", "*.m1a", "*.mp2", "*.m2a", "*.mpa", "*.mus", "*.mp3", "*.mpg", "*.mpeg",
            "*.mp3pro", "*.aif", "*.aiff", "*.bwf", "*.wma", "*.wmv", "*.aac", "*.adts", "*.mp4", "*.m4a", "*.m4b",
            "*.mod", "*.mdz", "*.mo3", "*.s3m", "*.s3z", "*.xm", "*.xmz", "*.it", "*.itz", "*.umx", "*.mtm", "*.flac",
            "*.fla", "*.oga", "*.ogg", "*.aac", "*.m4a", "*.m4b", "*.mp4", "*.mpc", "*.mp+", "*.mpp", "*.ac3", "*.wma",
            "*.ape", "*.mac"
        };
        ...
        {
            ".wav", ".ogg", ".mp1", ".m1a", ".mp2", ".m2a", ".mpa", ".mus", ".mp3", ".mpg", ".mpeg", ".mp3pro", ".aif",
            ".aiff", ".bwf", ".wma", ".wmv", ".aac", ".adts", ".mp4", ".m4a", ".m4b", ".mod", ".mdz", ".mo3", ".s3m",
            ".s3z", ".xm", ".xmz", ".it", ".itz", ".umx", ".mtm", ".flac", ".fla", ".oga", ".ogg", ".aac", ".m4a",
            ".m4b", ".mp4", ".mpc", ".mp+", ".mpp", ".ac3", ".wma", ".ape", ".mac"
        };
        ...
        /// <param name="skipDurationAboveSeconds">
        ///     Skip files with duration longer than this number of seconds (0 or less disables
        ///     this)
        /// </param>
            ...
            bool silent = false)
        {
            ...
            try
            {
                ...
                var filesAlreadyProcessed = repository.DatabaseService.ReadTrackFilenames();
                ...
                var filesRemaining = filesAll.Except(filesAlreadyProcessed).ToList();
                Console.Out.WriteLine("Found {0} files remaining in scan directory to be processed.",
                    filesRemaining.Count);

                var filesCounter = 1;
                var filesAllCounter = filesAlreadyProcessed.Count + 1;
                ...
                foreach (var file in filesRemaining)
                {
                    ...
                    var duration = bass.GetDurationInSeconds(fileInfo.FullName);
                        ...
                        || duration < 0)
                    {
                        if (!Analyzer.AnalyzeAndAddComplete(fileInfo, db, repository))
                        {
                            ...
                            Console.Out.WriteLine("Failed! Could not generate audio fingerprint for {0}!",
                                fileInfo.Name);
                        ...
                        else
                        {
                            Console.Out.WriteLine(
                                "[{1}/{2} - {3}/{4}] Succesfully added {0} to database. (Thread: {5})", fileInfo.Name,
                                filesCounter, filesRemaining.Count, filesAllCounter, filesAll.Count(),
                    ...
                    else
                    {
                        if (!silent)
                            Console.Out.WriteLine(
                                "Skipping {0} since duration exceeds limit ({1:0.00} > {2:0.00} sec.)", fileInfo.Name,
                                duration, skipDurationAboveSeconds);
                    }
...
#endif
                var filesActuallyProcessed = filesCounter - 1;
                Console.WriteLine("Added {0} out of a total remaining set of {1} files. (Of {2} files found).",
                    filesActuallyProcessed, filesRemaining.Count(), filesAll.Count());
            }
        ...
        {
            var analysisMethod = Analyzer.AnalysisMethod.SCMS;
            ...
            var queryId = -1;
            var numToTake = 20;
            var percentage = 0.4; // percentage below and above when querying
            var resetdb = false;
            var silent = false;
            var distanceType = AudioFeature.DistanceType.KullbackLeiblerDivergence;

            // Command line parsing
            var CommandLine = new Arguments(args);
            if (CommandLine["match"] != null) queryPath = CommandLine["match"];
            if (CommandLine["matchid"] != null)
            {
                var matchId = CommandLine["matchid"];
                queryId = int.Parse(matchId);
            }

            if (CommandLine["scandir"] != null) scanPath = CommandLine["scandir"];
            if (CommandLine["skipduration"] != null)
                double.TryParse(CommandLine["skipduration"], NumberStyles.Number, CultureInfo.InvariantCulture,
                    out skipDurationAboveSeconds);
            if (CommandLine["num"] != null)
            {
                var num = CommandLine["num"];
                numToTake = int.Parse(num);
            }

            if (CommandLine["percentage"] != null)
                double.TryParse(CommandLine["percentage"], NumberStyles.Number, CultureInfo.InvariantCulture,
                    out percentage);
            if (CommandLine["type"] != null)
            {
                var type = CommandLine["type"];
            ...
            if (CommandLine["dtw"] != null || CommandLine["dtwe"] != null)
                distanceType = AudioFeature.DistanceType.Dtw_Euclidean;
            if (CommandLine["dtwe2"] != null) distanceType = AudioFeature.DistanceType.Dtw_SquaredEuclidean;
            if (CommandLine["dtwman"] != null) distanceType = AudioFeature.DistanceType.Dtw_Manhattan;
            if (CommandLine["dtwmax"] != null) distanceType = AudioFeature.DistanceType.Dtw_Maximum;
            if (CommandLine["kl"] != null) distanceType = AudioFeature.DistanceType.KullbackLeiblerDivergence;
            if (CommandLine["ucrdtw"] != null) distanceType = AudioFeature.DistanceType.UCR_Dtw;
            if (CommandLine["resetdb"] != null) resetdb = true;
            if (CommandLine["silent"] != null) silent = true;
            if (CommandLine["permutations"] != null)
            {
                ...
                Console.WriteLine();
                var permutationGeneratorService = new PermutationGeneratorService();
            ...
            // Get database
            var mandelEllisScmsDatabase = new Db(); // For MandelEllis and SCMS

            // Instansiate soundfingerprinting Repository
            var fingerprintService = Analyzer.GetSoundfingerprintingService();
            var databaseService = DatabaseService.Instance; // For AudioFingerprinting
            ...
            if (queryPath != "")
            {
                ...
                if (fi.Exists)
                    FindSimilar(queryPath, mandelEllisScmsDatabase, analysisMethod, numToTake, percentage,
                        distanceType);
                else
            ...
            if (queryId != -1)
                FindSimilar(new[] { queryId }, mandelEllisScmsDatabase, analysisMethod, numToTake, percentage,
                    distanceType);
        ...
        private static void FindSimilar(int[] seedTrackIds, Db db, Analyzer.AnalysisMethod analysisMethod,
            int numToTake = 25, double percentage = 0.2,
            AudioFeature.DistanceType distanceType = AudioFeature.DistanceType.KullbackLeiblerDivergence)
        {
            var similarTracks = SimilarTracks(seedTrackIds, seedTrackIds, db, analysisMethod, numToTake, percentage,
                distanceType);
            foreach (var entry in similarTracks) Console.WriteLine("{0}, {1}", entry.Key, entry.Value);
        }

        private static void FindSimilar(string path, Db db, Analyzer.AnalysisMethod analysisMethod, int numToTake = 25,
        ...
        public static Dictionary<KeyValuePair<int, string>, double> SimilarTracks(string searchForPath, Db db,
            Analyzer.AnalysisMethod analysisMethod, int numToTake = 25, double percentage = 0.2,
            AudioFeature.DistanceType distanceType = AudioFeature.DistanceType.KullbackLeiblerDivergence)
        {
            var t = new DbgTimer();
            t.Start();

            var fi = new FileInfo(searchForPath);
            ...
            var r = db.GetTracks(null, seedAudioFeature.Duration, percentage);

            // store results in a dictionary
            var NameDictionary = new Dictionary<KeyValuePair<int, string>, double>();

            var mapping = new int[100];
            var read = 1;
            double dcur;
            ...
            var sortedDict = (from entry in NameDictionary orderby entry.Value select entry)
                .Take(numToTake)
                .ToDictionary(pair => pair.Key, pair => pair.Value);

            Console.Out.WriteLine("Found Similar to ({0}) in {1} ms", seedAudioFeature.Name,
        ...
        public static List<QueryResult> SimilarTracksList(string searchForPath, Db db,
            Analyzer.AnalysisMethod analysisMethod, int numToTake = 25, double percentage = 0.2,
            AudioFeature.DistanceType distanceType = AudioFeature.DistanceType.KullbackLeiblerDivergence)
        {
            var fi = new FileInfo(searchForPath);
        ...
        /// <param name="distanceType">distance method to use (KullbackLeiblerDivergence is default)</param>
        /// <returns>a dictinary list of key value pairs (filepath and distance)</returns>
        public static Dictionary<KeyValuePair<int, string>, double> SimilarTracks(int[] id, int[] exclude, Db db,
            Analyzer.AnalysisMethod analysisMethod, int numToTake = 25, double percentage = 0.2,
            AudioFeature.DistanceType distanceType = AudioFeature.DistanceType.KullbackLeiblerDivergence)
        {
            var t = new DbgTimer();
            ...
            for (var i = 0; i < seedAudioFeatures.Length; i++)
                seedAudioFeatures[i] = db.GetTrack(id[i], analysisMethod);

            // Get all tracks from the DB except the seedSongs
            var r = db.GetTracks(exclude, seedAudioFeatures[0].Duration, percentage);
            ...
            while (read > 0)
            {
                ...
                for (var i = 0; i < read; i++)
                {
                    d = 0;
                    count = 0;
                    for (var j = 0; j < seedAudioFeatures.Length; j++)
                    {
            ...
            Console.Out.WriteLine("Found Similar to ({0}) in {1} ms",
                string.Join(",", seedAudioFeatures.Select(p => p.Name)), t.Stop().TotalMilliseconds);
            return sortedDict;
        }
        ...
        /// <param name="distanceType">distance method to use (KullbackLeiblerDivergence is default)</param>
        /// <returns>a  list of query results</returns>
        public static List<QueryResult> SimilarTracksList(int[] id, int[] exclude, Db db,
            Analyzer.AnalysisMethod analysisMethod, int numToTake = 25, double percentage = 0.2,
            AudioFeature.DistanceType distanceType = AudioFeature.DistanceType.KullbackLeiblerDivergence)
        {
            AudioFeature[] seedAudioFeatures = null;
        ...
        public static void Compare(string path1, string path2, Analyzer.AnalysisMethod analysisMethod)
        {
            ...
            var filePath1 = new FileInfo(path1);
            var filePath2 = new FileInfo(path2);
            ...
            Console.Out.WriteLine("Similarity between m1 and m2 is: "
                                  + m1.GetDistance(m2));
        }
        ...
        {
            var m1 = db.GetTrack(trackId1, analysisMethod);
            var m2 = db.GetTrack(trackId2, analysisMethod);
