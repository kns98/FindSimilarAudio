...
namespace Imghash
{
    ...
    {
        private readonly int size = 32; // 32 (64 usable?)
        private readonly int smallerSize = 8; // 8  (16 usable?)
        ...
        public static int HammingDistance(string s1, string s2)
        {
            ...
            var length = lengthS2 > lengthS1 ? lengthS1 : lengthS2;
        ...
        {
            var different = HammingDistance(s1, s2);
            var perc = different / (double)s1.Length;
            return 100 * (1 - perc);
        }
        ...
        public string GetHash(Bitmap img)
        {
            ...
            for (var x = 0; x < img.Width; x++)
            {
                ...
                for (var y = 0; y < img.Height; y++) // when the image is grayscale RGB has the same value
            ...
            for (var y = 0; y < size; y++) // add to byte array
                grayscaleByteArray[x + y * size] = Convert.ToByte(vals[x][y]);
            var grayscale = ImageUtils.ByteArrayGrayscaleToImage(grayscaleByteArray, size, size);
            ...
            //	a 32x32 DCT.
            var dctVals = DctMethods.dct2(vals);
            ...
            for (var x = 0; x < size; x++)
            {
                ...
                for (var y = 0; y < size; y++) dctLogVals[x][y] = Math.Log(1 + Math.Abs((int)dctVals[x][y]));
            }
            ...
            for (var y = 0; y < size; y++)
            {
                ...
                var dctPixelVal = (int)((float)(dctLogVals[x][y] - dctMin) / (float)(dctMax - dctMin) * 255);
                dctPixels[x + y * size] = Convert.ToByte(dctPixelVal);
            }

            var dctImage = ImageUtils.ByteArrayGrayscaleToImage(dctPixels, size, size);
            ...
            for (var y = 0; y < smallerSize; y++)
            {
                ...
                var dctPixelSmallerVal = (int)((float)(dctLogVals[x][y] - dctMin) / (float)(dctMax - dctMin) * 255);
...
#if DEBUG
            var dctImageSmaller = ImageUtils.ByteArrayGrayscaleToImage(dctPixelsSmallerSize, smallerSize, smallerSize);
            dctImageSmaller.Save("ImagePHash 6-dct-smaller.png");

            // Inverse DCT
            var inverseDctVals = DctMethods.idct2(dctValsOnlyHighFreq);

            var idctPixels = new byte[size * size];
            for (var x = 0; x < size; x++)
            for (var y = 0; y < size; y++)
            {
                var inverseDctVal = inverseDctVals[x][y];
            ...
            var idctImage = ImageUtils.ByteArrayGrayscaleToImage(idctPixels, size, size);
            idctImage.Save("ImagePHash 7-idct.png");
#endif

            // 5. b) Compute the average value.
            var avg = total / (smallerSize * smallerSize - 1);
