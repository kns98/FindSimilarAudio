...
namespace UCRCSharp
{
    public partial class UCR
    {
        ...
        private static double distance(double[] Q, double[] T, long j, long m, double mean, double std, long[] order,
            double bsf)
        {
            int i;
            double sum = 0;
                ...
                using (qp = File.OpenRead(queryFileName))
                {
                    ...
                    using (TextReader reader = new StreamReader(qp))
                    {
                        ...
                        while ((line = reader.ReadLine()) != null)
                        {
                            ...
                            for (var k = 0; k < strs.Length && i < m; k++)
                            {
