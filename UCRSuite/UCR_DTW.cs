...
namespace UCRCSharp
{
    ...
    {
        private static void bubble_sort_generic<T>(T[] array) where T : IComparable
        {
            ...
            using (qp = File.OpenRead(queryFileName))
            {
                using (TextReader reader = new StreamReader(qp))
                {
                    ...
                    while ((line = reader.ReadLine()) != null)
                    {
                        ...
                        for (var itt = 0; itt < strs.Length && i < m; itt++)
                        {
        ...
        public static double DTW(double[] inputArray, double[] queryArray, double wrappingWindow = 0)
        {
            ...
            for (var itt = 0; itt < queryArray.Length; itt++)
            {
                d = queryArray[itt];
                ex += d;
                ex2 += d * d;
            ...
            for (var idata = 0; idata < inputArray.Length; idata++) data.Enqueue(inputArray[idata]);
            ...
            double bsf = double.PositiveInfinity)
        {
            ...
            double d, lb;
            var x0 = (t[j] - mean) / std;
            var y0 = (t[len - 1 + j] - mean) / std;
            lb = dist(x0, q[0]) + dist(y0, q[len - 1]);
            if (lb >= bsf) return lb;

            /// 2 points at front
            var x1 = (t[j + 1] - mean) / std;
            ...
            double best_so_far = double.PositiveInfinity)
        {
            ...
            for (var i = 0; i < len && lb < best_so_far; i++)
            {
            ...
            double bsf = double.PositiveInfinity)
        {
            ...
            for (i = 0; i < m; i++)
            {
                ...
                if (i + r < m - 1 && min_cost + cb[i + r + 1] >= bsf) return min_cost + cb[i + r + 1];
