...
namespace Soundfingerprinting.Hashing
{
    public class MinHash : IHashingService<bool[], int[]>
    {
        ...
        public int[] ComputeMinHashSignature(bool[] fingerprint)
        {
            ...
            var minHash = new int[permutations.Length]; /*100*/
            for (var i = 0; i < permutations.Length /*100*/; i++)
            {
                minHash[i] = 255; /*The probability of occurrence of 1 after position 255 is very insignificant*/
                var len = permutations[i].Length;
                for (var j = 0; j < len /*256*/; j++)
        ...
        public Dictionary<int, long> GroupMinHashToLSHBuckets(int[] minHashes, int numberOfHashTables,
            int numberOfMinHashesPerKey)
        {
            ...
            for (var i = 0; i < numberOfHashTables /*hash functions*/; i++)
            {
                var array = new byte[MaxNumber];
                for (var j = 0; j < numberOfMinHashesPerKey /*r min hash signatures*/; j++)
                    array[j] = (byte)minHashes[i * numberOfMinHashesPerKey + j];

                var hashbucket = BitConverter.ToInt64(array, 0); // actual value of the signature
        ...
        {
            var aarray = BitConverter.GetBytes(aBucket);
            var barray = BitConverter.GetBytes(bBucket);
