...
namespace Soundfingerprinting.DbStorage
{
    ...
    public class DatabaseService
    {
        ...
        protected DatabaseService()
        {
            ...
            var dbfile = Path.Combine(dbdir, "soundfingerprinting.db");
        ...
        public IList<Fingerprint> ReadFingerprintsByTrackId(int trackId, int numberOfFingerprintsToRead)
        {
            ...
            dbcmd.CommandText =
                "SELECT id, songorder, signature FROM [fingerprints] WHERE [trackid] = @trackid LIMIT @limit";
            ...
            IEnumerable<Track> tracks, int numberOfFingerprintsToRead)
        {
            ...
            var statementValueTags = string.Join(",", tracks.Select(x => x.Id));
            ...
            var query = string.Format(
                "SELECT id, trackid, songorder, signature FROM [fingerprints] WHERE (trackid IN ({0})) LIMIT {0};",
                statementValueTags, numberOfFingerprintsToRead);
            ...
            if (lastTrackId != -1) // add last fingerprints
                result.Add(lastTrackId, fingerprints);
        ...
        public Fingerprint ReadFingerprintById(int id)
        {
            ...
            dbcmd.CommandText =
                "SELECT trackid, songorder, totalfingerprints, signature FROM [fingerprints] WHERE [id] = @id";
        ...
        public IList<Fingerprint> ReadFingerprintById(IEnumerable<int> ids)
        {
            ...
            var statementValueTags = string.Join(",", ids);

            var query = string.Format(
                "SELECT id, trackid, songorder, totalfingerprints, signature FROM [fingerprints] WHERE (id IN ({0}));",
                statementValueTags);
            dbcmd.CommandText = query;
            dbcmd.CommandType = CommandType.Text;
            dbcmd.Prepare();
        ...
        public IList<Track> ReadTracks()
        {
            ...
            while (reader.Read())
            {
                ...
                if (!reader.IsDBNull(3)) track.Artist = reader.GetString(3);
        ...
        public IList<Track> ReadTracks(string whereClause)
        {
            ...
            var query = "SELECT id, albumid, length, artist, title, filepath FROM [tracks]";
        ...
        public Track ReadTrackById(int id)
        {
            ...
            if (!reader.IsDBNull(2)) track.Artist = reader.GetString(2);
        ...
        public IList<Track> ReadTrackById(IEnumerable<int> ids)
        {
            ...
            var query = string.Format(
                "SELECT id, albumid, length, artist, title, filepath FROM [tracks] WHERE (id IN ({0}));",
                statementValueTags);
            dbcmd.CommandText = query;
            dbcmd.CommandType = CommandType.Text;
            dbcmd.Prepare();
        ...
        public IDictionary<int, IList<HashBinMinHash>> ReadFingerprintsByHashBucketLshSlow(long[] hashBuckets)
        {
            ...
            dbcmd.CommandText =
                "SELECT id, hashbin, hashtable, trackid, fingerprintid FROM hashbins WHERE hashbin = @hashbin";
            ...
            foreach (var hashBin in hashBuckets)
            {
                ...
                while (reader.Read())
                {
                    var hashId = reader.GetInt32(0);
                    long hashBin2 = reader.GetInt32(1);
                    var hashTable = reader.GetInt32(2);
                    var trackId = reader.GetInt32(3);
                    var fingerprintId = reader.GetInt32(4);
        ...
        public IDictionary<int, IList<HashBinMinHash>> ReadFingerprintsByHashBucketLsh(long[] hashBuckets)
        {
            ...
            var statementValueTags = string.Join(",", hashBuckets);
            ...
            var query = string.Format(
                "SELECT id, hashbin, hashtable, trackid, fingerprintid FROM hashbins WHERE (hashbin IN ({0}))",
                statementValueTags);
            dbcmd.CommandText = query;
            dbcmd.CommandType = CommandType.Text;
            dbcmd.Prepare();
        ...
        public IDictionary<int, IList<HashBinMinHash>> ReadAllFingerprints()
        {
            ...
            var query = "SELECT id, hashbin, hashtable, trackid, fingerprintid FROM hashbins";
            dbcmd.CommandText = query;
            dbcmd.CommandType = CommandType.Text;
            dbcmd.Prepare();
        ...
        private static bool[] ByteToBool(byte[] byteArray)
        {
            ...
            for (var i = 0; i < byteArray.Length; i++) boolArray[i] = byteArray[i] == 1 ? true : false;
            return boolArray;
        }
        ...
        private static byte[] BoolToByte(bool[] boolArray)
        {
            ...
            // basic - same count
            var byteArray = Array.ConvertAll(boolArray, b => b ? (byte)1 : (byte)0);
            return byteArray;
        }
