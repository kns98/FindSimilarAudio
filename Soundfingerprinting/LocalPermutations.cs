...
namespace Soundfingerprinting.Hashing
{
    ...
    public class LocalPermutations : IPermutations
    {
        ...
        public int[][] GetPermutations()
        {
            ...
            using (var reader = new StreamReader(_pathToPerms))
            {
                while (reader.Peek() != -1)
                {
                    ...
                    if (line != null)
                    {
                        var ints = line.Split(new[] { _separator }, StringSplitOptions.RemoveEmptyEntries);
                        var permutation = new int[ints.Length];
                        var i = 0;
